...
class ScraperOptions:
  ...
  Properties:
    showBrowser: bool, property specifying whether to show the browser
    when logging into mint.
    cookies: Opt. location of the cookies to load into the browser session.
    scrapeTransactions: If true, scrapes all txns from the account.
    scrapeAccounts: If true, scrapes all financial account data from the accounts.
  """
  ...
  def fromArgs(cls, args: argparse.Namespace) -> 'ScraperOptions':
    ...
    if args.debug:
      return ScraperOptions(
          args.types, showBrowser=args.debug, cookies=args.cookies)
    else:
      return ScraperOptions(args.types, cookies=args.cookies)
...
def scrape_and_upload(options: ScraperOptions) -> None:
  """
  Launches a MintApi scraper to fetch txn data and upload to 
  Google sheets.

  Args:
      options: The settings to use when running the scrape.

  Env:
      Requires environment variables from chromedriver, etc. as well as
      credential informaiton. See README or _GetCredentials.
  ...
  if options.cookies:
    cookies_file = os.path.join(
        chromedriver_download_path, options.cookies)
