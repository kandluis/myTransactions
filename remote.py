...
def LogIntoMint(creds: auth.Credentials, options: utils.ScraperOptions,
                cookies: Iterable[str]) -> mintapi.Mint:
...
def RetrieveTransactions(
  ...
  cutoff: str = old_txns.Date[
      old_txns.Date.size - config.GLOBAL.NUM_TXN_FOR_CUTOFF]
  ...
  spend_txns = txns[
      (~txns['accountRef.name'].isin(
          config.GLOBAL.SKIPPED_ACCOUNTS
      )) & txns.isExpense]
  ...
  spend_txns = spend_txns[~(
      spend_txns.Category.isin(
          config.GLOBAL.IGNORED_CATEGORIES
...
def UpdateGoogleSheet(sheet: pygsheets.Spreadsheet,
  ...
  settings_ws = sheet.worksheet_by_title(
      title=config.GLOBAL.SETTINGS_SHEET_TITLE)
